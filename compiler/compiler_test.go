...
func testConstants(t *testing.T, expected []interface{}, actual []object.Object) error {
	...
	if len(expected) != len(actual) {
		t.Fatalf("wrong number of constants. want=%d, got=%d",
...
func TestFunctionCalls(t *testing.T) {
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedInstructions: []code.Instructions{
				code.Make(code.OpClosure, 1), // compiled function
				code.Make(code.OpSetGlobal, 0),
				code.Make(code.OpGetGlobal, 0),
				code.Make(code.OpCall, 0),
				code.Make(code.OpPop),
...
func TestCompilerScopes(t *testing.T) {
	compiler := New()
	if compiler.scopeIndex != 0 {
		t.Errorf("compiler.scopeIndex wrong. got=%d, want=%d", compiler.scopeIndex, 0)
	}

	globalSymtablTable := compiler.symbolTable
	compiler.emit(code.OpMul)

	compiler.enterScope()
	if compiler.scopeIndex != 1 {
		t.Errorf("compiler.scopeIndex wrong. got=%d, want=%d", compiler.scopeIndex, 1)
	}
	...
	if compiler.symbolTable.Outer != globalSymtablTable {
		t.Errorf("compiler did not enclose symbol table")
	}

	compiler.leaveScope()
	if compiler.scopeIndex != 0 {
		t.Errorf("compiler.scopeIndex wrong. got=%d, want=%d",
			compiler.scopeIndex, 0)
	}

	if compiler.symbolTable != globalSymtablTable {
